...
class CreateEmbedding(MTimeMixin, luigi.Task):
    ...
    def run(self):
        ...
        tsne = TSNE(
            ...
            angle=config.getfloat("PreprocessingConstants", "tsne_theta"),
            verbose=2
        )
        out = tsne.fit_transform(vectors)
...
def test_CreateEmbedding_task():
    ...
    for i in range(3):
        ...
        vecsdf = pd.read_table(config.getSample("ExternalFiles", "vecs_with_id"), skiprows=1,
...
def test_createCoordinates_task():
    ...
    vecs = Utils.read_vectors(config.get("ExternalFiles", "vecs_with_id"))  # Each entry of vectors in a column
